...
# Processes that routinely open ephemeral ports — suppress port alerts for these.
_SAFE_PORT_PROCS: frozenset[str] = frozenset({
    "code", "code.exe",                      # VSCode main
    "code - insiders", "code - insiders.exe",
    "code helper", "code helper (renderer)",  # VSCode renderer workers
    "electron", "electron.exe",
    "node", "node.exe",                       # Node.js (extensions, LSP servers)
    "python", "python.exe", "python3",        # YUI itself + any python tool
    "chrome", "chrome.exe",
    "msedge", "msedge.exe",
    "firefox", "firefox.exe",
    "brave", "brave.exe",
    ...
    "teams", "teams.exe",
    "zoom", "zoom.exe",
    "svchost", "svchost.exe",                 # Windows system services
    "system",
})


def _is_safe_port_proc(pname: str) -> bool:
    low = pname.lower().split("(")[0].strip()
    return any(low == s or low.startswith(s.rstrip(".exe")) for s in _SAFE_PORT_PROCS)
