"""
persona.py — Personality, tone, style enforcement, macro shortcuts, teaching mode.
"""
...
class StylePolicy:
    """Learns user style preferences and enforces them on every response."""
...
class MacroPolicy:
    """Learns 'when I say X, do Y' shortcuts and expands them at input time."""
    ...
    def maybe_handle_training(self, user_text: str) -> Optional[str]:
        ...
        if m:
            return self._save_macro(m.group(1), m.group(2))

        m2 = re.match(r"^aprende\s+(?:comando|atajo)\s+(.+?)\s*(?:=>|=|->)\s*(.+)$", t, flags=re.IGNORECASE)
        if m2:
            return self._save_macro(m2.group(1), m2.group(2))
...
class TeachingModePolicy:
    """Controls teaching mode: detailed text output with concise TTS version."""
    ...
    def maybe_handle(self, user_text: str) -> Optional[str]:
        ...
        if low in {"modo enseñanza", "modo ensenanza", "activa enseñanza", "activar enseñanza",
                   "activa ensenanza", "activar ensenanza", "enseñanza on", "ensenanza on"}:
            self.enabled = True
            self._save()
            return "Listo. Modo enseñanza activado."

        if low in {"modo normal", "desactiva enseñanza", "desactivar enseñanza",
                   "desactiva ensenanza", "desactivar ensenanza", "enseñanza off", "ensenanza off"}:
            self.enabled = False
            self._save()
            return "Listo. Modo enseñanza desactivado."

        if low in {"enseñanza", "ensenanza", "estado enseñanza", "estado ensenanza",
                   "enseñanza estado", "ensenanza estado"}:
